...
public class Main {
    ...
    // Lists the options a user may do after selecting another user.
    ...
    // Print main menu where user can choose what to do.
    private static void runMainMenu(Scanner scan, AccountsMaster accountsMaster, User loggedOnUser) {
        while (true) {
            ...
            // Lets Sellers view their stores and edit them. Only available to Sellers.
    ...
    // Opens options after selecting a conversation.
    // Allows sending messages and downloading conversation as a csv file.
        ...
        if (conversations.size() > 0) {
            while (true) {
                ...
                if (conversationNumber == conversations.size() + 2) {
                    ...
                    //  Exports selected conversations as csv files into desired folder path.
                    // Catches exceptions and prints errors when improper inputs are used.
                    try {
    ...
    // Displays options to use on a conversation's messages.
        ...
        if (!(AccountsMaster.sellerArrayList.contains(recipient) || AccountsMaster.customerArrayList.contains((recipient)))) {
            ...
            while (true) {
                ...
                if (conversationAction.contains(".")) {
                    ...
                    // Edits a message and rewrites it in conversation file.
                    if (action == 5) {
                    ...
                    // Prints out the messages in the conversations
                    if (action == 1) {
                        for (int i = 0; i < messages.size(); i++) {
                        ...
                        // Import a text file and sends the contents as a message
                        // Does not send a message if the user is blocked.
                        if (!recipient.getBlockedUsers().contains(loggedOnUser) &&
                                !loggedOnUser.getBlockedUsers().contains(recipient)) {
