...
#### 3.2 `AccountsMaster`
The `AccountsMaster` class acts as the helper class to `Main`. It handles the creation and deletion of accounts. It lists conversations, sellers, stores, and customers as per `this User`.

##### `public static void replaceStringInFile(String filePath, String oldString, String newString)`
Method that replaces an old Conversation or User string with its updated version in the file at `filePath`. This is used to keep all objects updated during `Main`'s execution.
...
Method that reads the file according to the `User user`'s visibility of each `Message` (i.e. they cannot see the message if they have deleted it).
...
Method that creates a new `Message` object and appends that `Message` object String to the `Conversation` file with the messages.
